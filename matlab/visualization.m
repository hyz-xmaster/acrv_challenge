...
switch mode
  ...
  case 'init'
    ...
    fig = figure(1);
    img = imread(sprintf('%s/%08d.png',image_dir,0));
  ...
  case 'update'
    ...
    % read image
    img = imread(sprintf('%s/%08d.png',image_dir,img_idx));
    image_size = [size(img, 1), size(img,2)];
    varargout{2} = image_size;
    ...
    text(0,60,'Severely occluded','parent',h(1).axes,'color','r','HorizontalAlignment','left','VerticalAlignment','top','FontSize',14,'FontWeight','bold','BackgroundColor','black');
